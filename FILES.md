...
| File | Role |
|---|---|
| `Panel.qml` | The shell entry point. Owns plugin state and helper paths, the theme palette roles, the scale/fullscreen math, the input aggregation, the 16 ms tick, the auto-pause when the panel is closed, and the bar label (`N_target` progress while a round is active — paused included, so the slot never resizes — the score in collect modes, `ö_Ö` when idle). Input is one place on purpose: `p1Left/Right/JumpHeld` and `p2Left/Right/JumpHeld` mix `keysDown` with the pad fields, and the keyboard layout lives there — P1 = WASD + Space, P2 = arrows + Return/numpad Enter, with the arrows folded into P1 while P2 has not joined (`p2PadInput()` is what can join, so an arrow key cannot split the screen). It instantiates the components below and owns no view ids: `ui/GamePanel.qml` reports its chrome heights (`hdrRowH`, `btnRowH`, `helpSectionH`, `helpToggleH`, `padToggleH`, `boardSlotH`) for `_chromeH`/`_helpBudget`. Pad input arrives on the bridge's stdout — capped per line, every field re-coerced in `applyPadLine`. The tick turns the pad's edges into actions: `pause` (Start — pause a live round, start one from the ready screen), `menu` (Select/Mode), `fullscreen` (R3, and only while the panel is open) on any pad, `up`/`down`/`confirm` for the picker, all rising-edge and with the flags kept hot while idle so a held button cannot fire on open. |
...
| `core/Keys.js` | The keyboard layout as pure functions: `p1Left/Right/Jump`, `p2Left/Right/Jump` and `p2PadInput`, each taking the live key set, the pad state and whether P2 is in the round. P1 = A/D + W or Space, P2 = arrows + Up or Enter (Return and numpad Enter), and while P2 has not joined the arrows/Enter are folded into P1. `p2PadInput` is the only join trigger, so a solo arrow press cannot split the screen. The one deadzone constant lives here too. |
...
| `ui/GameBoard.qml` | Arena layout: one `BoardView` solo, two side by side with a divider when P2 joins, plus the idle, paused and win overlays (glyph art only — the win marker is the finish band's `▀▄▀▄` stripe). The cards name each action's key, so their key lines are the one place besides the README that has to follow a remap. Read-only over the engine. |
...
| `ui/BarButton.qml` | The bar icon (`BarIconButton`): sizes its slot from the live label via `TextMetrics`. That is the constraint to respect — any label change resizes the slot and moves the panel, which is anchored here — so transient state (running/paused, pad ×N) goes into the tooltip instead, flattened through `ui/Plain.js` because the bar renders it. Left click toggles the panel, right click asks for the reload menu. |
